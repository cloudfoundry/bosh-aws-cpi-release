  ...
  describe InstanceParamMapper do
    ...
    describe '#instance_params' do
      ...
      describe 'tags' do
        ...
        context 'when tags are supplied via vm_type cloud properties' do
          let(:vm_type) { { 'tags' => { 'cloud_prop_tag' => 'cloud_prop_value' } } }

          it 'populates the tags attribute on VMCloudProps from cloud properties' do
            expect(vm_cloud_props.tags).to eq({ 'cloud_prop_tag' => 'cloud_prop_value' })
          end
        end

        context 'when no tags are supplied' do
          it 'returns an empty hash for VMCloudProps tags' do
            expect(vm_cloud_props.tags).to eq({})
          end
        end
      end
    end
